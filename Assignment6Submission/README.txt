Joshua A. Barrs
002294736
barrs@chapman.edu 
CPSC 350 - 01 
Assignment 6: Sorting 

Source files being submitted: 

	BubbleSortAlgorithm.h
	BubbleSortAlgorithm.cpp
	CWU Math335 Article Summary.zip (the LaTex report)
	CWU_Math335_Article_Summary.pdf (pdf version of LaTex report)
	DoubleValues.txt
	InsertionSortAlgorithm.h
	InsertionSortAlgorithm.cpp
	Makefile
	QuickSort.h
	QuickSort.cpp
	SelectionSortAlgorithm.h
	SelectionSorthAlgorithm.cpp
	SortData.h
	SortData.cpp
	SortingMain.cpp

Compile/Run-Time Errors: 
	No known compile or run-time errors 

References Used to Complete the Assignment: 

	https://www.tutorialspoint.com/cplusplus/cpp_passing_arrays_to_functions.htm
	http://www.cplusplus.com/reference/string/string/getline/
	http://www.cplusplus.com/reference/string/stod/
	http://www.cplusplus.com/reference/stdexcept/invalid_argument/
	http://www.algolist.net/Algorithms/Sorting/Quicksort
	http://www.cplusplus.com/reference/array/array/size/
	https://stackoverflow.com/questions/13400921/error-invalid-types-doubledouble-for-array-subscript
	https://www.overleaf.com/learn/how-to/Downloading_a_Project
	http://www1.cmc.edu/pages/faculty/aaksoy/latex/latexthree.html
	http://www.cplusplus.com/reference/ctime/localtime/
	https://www.geeksforgeeks.org/selection-sort/
	https://www.geeksforgeeks.org/insertion-sort/
	https://www.geeksforgeeks.org/bubble-sort/
	https://stackoverflow.com/questions/33523585/how-do-sizeofarr-sizeofarr0-work
	https://stackoverflow.com/questions/41077377/how-to-get-current-time-in-milliseconds/41077420

Instructions for running the assignment: 
	Compile all source files with the makefile provided
	Run the program (./a.out)